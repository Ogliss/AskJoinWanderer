	...
	public static class WJP_Wanderer_Joins_Patch
	{
		...
		//	Log.Message("WJP_IncidentWorker checking "+ __instance.def.defName);
			if (__instance.def.defName.Contains("WandererJoin") && !__instance.def.defName.Contains("ManInBlack") && !__instance.def.defName.Contains("StrangerInBlack"))
			{
				...
				PawnGenerationRequest pawnGenerationRequest = new PawnGenerationRequest(pawnKind, ofPlayer, (PawnGenerationContext)2, -1, true, false, false, true, pawnMustBeCapableOfViolence, num, false, true, true, false, false, false, false, false);
				Pawn pawn = PawnGenerator.GeneratePawn(pawnGenerationRequest);
                if (__instance.GetType().Name.Contains("Mime"))
                {
					pawn.health.AddHediff(HediffDef.Named("AA_MimeHediff"), null, null, null);
				}
				TaggedString ttexta = new TaggedString();
